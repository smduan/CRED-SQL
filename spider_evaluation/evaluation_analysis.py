...
def evaluate(gold, predict, db_dir, etype, kmaps, plug_value, keep_distinct, progress_bar_for_each_datapoint):
    ...
    assert len(plist) == len(glist), "number of sessions must equal, plist {}, glist {}".format(len(plist),len(glist))
    ...
    if etype in ["all", "exec"]:
        print('=====================   EXECUTION ACCURACY     =====================\n')
        exec_scores = [scores[level]['exec'] for level in levels]
        template = "{:20} " + ' '.join(['{:<20.3f}'] * len(levels))
        print(template.format("execution\n", *exec_scores))
