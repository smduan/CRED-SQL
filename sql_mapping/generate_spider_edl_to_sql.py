...
def generate_sql(model_id, message_path, log_path,output_sql_path):
    ...
    for message in messages:
        instruction = "You are a SQL expert. Given the SQL generation description, output final sqlite SQL query.\nSQL description:\n"+message['edl']+"\nOnly output SQL without any other text:"
        ...
        item = {"index":i,"description":message['edl'],"sql": sql}
    ...
    log.close()
    output_sql.close()
    with open(output_sql_path.replace(".txt",".json"), "w") as f:
